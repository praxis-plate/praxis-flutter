...
class Task {
  final String title;
  final String textInMarkdown;

  Task({
    required this.title,
    required this.textInMarkdown,
  });
...
class Theory extends Task {
  Theory({
    required super.title,
    required super.textInMarkdown,
  });

  @override
  String toString() => 'Теория';
}

class Practice extends Task {
  Practice({
    required super.title,
    required super.textInMarkdown,
  });
...
class Course {
  ...
  Course({
    ...
    this.ratingScores,
    this.description,
  })  : _tasksCount = taskNodes.length,
        _lessonsCount = taskNodes.whereType<Theory>().length,
        _practicesCount = taskNodes.whereType<Practice>().length;

  String tableOfContents() {
    String result = '';
    for (int i = 0; i < taskNodes.length; i++) {
      final task = taskNodes[i].task;
